...
void SAMPLE_SVP_Usage(char* pchPrgName)
{
    ...
    printf("\t 8) Yolov3(model file, image file).\n");
...
#endif
    HI_CHAR* bgr_file=argv[3];
    HI_CHAR* modelfile=argv[2];
    switch (*argv[1])
    {
        case '0':
            {
        ...
        case '8':
            {
                SAMPLE_SVP_NNIE_Yolov3(modelfile,bgr_file);
            }
            break;
        case '9':
            {
        ...
        case 'b': //yolov5
            {
                SAMPLE_SVP_NNIE_Yolov5(bgr_file);
            }
        default :
            {
                SAMPLE_SVP_Usage(argv[0]);
