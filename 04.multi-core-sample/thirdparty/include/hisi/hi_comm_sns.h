...
typedef struct hiISP_CMOS_ISP_SHARPEN_S
{
    HI_BOOL  bInvalid;
    ISP_SHARPEN_MANUAL_ATTR_S stManual;
    ISP_SHARPEN_AUTO_ATTR_S   stAuto;

} ISP_CMOS_ISP_SHARPEN_S;

typedef struct hiISP_CMOS_ISP_EDGEMARK_S
{
    HI_BOOL bValid;
...
#define PREGAMMA_NODE_NUMBER   257 //Update NODE NUMBER
typedef struct hiISP_CMOS_PREGAMMA_S
{
    HI_BOOL bValid;         /* wdr sensor should set */
    HI_BOOL bEnable;
    HI_U32  au32PreGamma[PREGAMMA_NODE_NUMBER];
...
typedef struct hiISP_CMOS_deGAMMA_S
{
    HI_BOOL bValid;

    HI_U16  au16deGamma[GAMMA_NODE_NUMBER];
} ISP_CMOS_DEGAMMA_S;
...
typedef struct hiISP_CMOS_LSC_S
{
    HI_BOOL bValid;
    HI_U8 u8MeshScale;
    ISP_LSC_CABLI_TABLE_S stLscCalibTable[2];
} ISP_CMOS_LSC_S;
...
typedef struct hiISP_CMOS_CA_S
{
    ...
    HI_S32    as32ISORatio[ISP_AUTO_ISO_STRENGTH_NUM];          //1.10bit  ISO Ratio  For UV ,Max = 2047 FW Limi
    HI_BOOL   bCpEnable;
...
typedef struct hiISP_CMOS_DEFAULT_S
{
    ...
    ISP_CMOS_PREGAMMA_S        stPreGamma;
    ISP_CMOS_GAMMA_S           stGamma;
    ISP_CMOS_DEGAMMA_S         stdeGamma;
    ISP_CMOS_ISP_SHARPEN_S     stIspSharpen;
    ISP_CMOS_ISP_EDGEMARK_S     stIspEdgeMark;
    ISP_CMOS_LDCI_S            stLdci;
    ISP_CMOS_DPC_S             stDpc;
    ISP_CMOS_LSC_S             stLsc;
    ISP_CMOS_RLSC_S            stRLsc;
    ISP_CMOS_GE_S              stGe;
    ISP_CMOS_ANTIFALSECOLOR_S  stAntiFalseColor;
    ...
    ISP_CMOS_CLUT_S            stClut;
    ISP_CMOS_LOGLUT_S          stLogLUT;
    ISP_CMOS_PRELOGLUT_S       stPreLogLUT;
    ISP_CMOS_SENSOR_MODE_S     stSensorMode;
    ISP_CMOS_WDR_S             stWdr;
...
typedef struct hiISP_SENSOR_EXP_FUNC_S
{
    ...
    HI_U32(*pfn_cmos_get_isp_default)(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef);
    HI_U32(*pfn_cmos_get_isp_black_level)(VI_PIPE ViPipe, ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel);
