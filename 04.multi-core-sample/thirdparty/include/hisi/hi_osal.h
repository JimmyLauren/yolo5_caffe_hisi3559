...
extern int osal_down_timeout(osal_semaphore_t *sem, long ms);
extern void osal_up(osal_semaphore_t *sem);
extern void osal_sema_destory(osal_semaphore_t *sem); //notice:must be called when kmod exit, other wise will lead to memory leak;
...
extern void osal_mutex_destory(osal_mutex_t *mutex); //notice:must be called when kmod exit, other wise will lead to memory leak;
...
extern int osal_spin_trylock_irqsave(osal_spinlock_t *lock, unsigned long *flags);
extern void osal_spin_unlock_irqrestore(osal_spinlock_t *lock, unsigned long *flags);
extern void osal_spin_lock_destory(osal_spinlock_t *lock); //notice:must be called when kmod exit, other wise will lead to memory leak;

//wait api
typedef int (*osal_wait_cond_func_t)(void *param);
...
extern int osal_wait(osal_wait_t * wait, osal_wait_cond_func_t func, void* param);
extern int osal_wait_uninterruptible(osal_wait_t * wait, osal_wait_cond_func_t func, void* param);

extern int osal_wait_timeout(osal_wait_t * wait, osal_wait_cond_func_t func, void* param, unsigned long ms);

//dev_wait_event-->wait_event_interruptible
#define osal_wait_event(wait, func, param)    \
({                                  \
    int __ret = 0;                          \
                                          \
    for (;;){                          \
        ...
        __ret = osal_wait_timeout(wait, (func), param, 100);   \
        if(__ret < 0)                 \
            break;           \
    }                                    \
    __ret;                                   \
...
//osal_wait_event_interruptible_timeout-->wait_event_interruptible_timeout
#define osal_wait_event_timeout(wait, func, param, timeout)             \
({                                  \
    int __ret = timeout;                          \
                                        \
    ...
    for (;;) {                          \
        ...
        __ret = osal_wait_timeout(wait, (func), param, __ret);   \
...
extern int osal_register_gic_handle(unsigned int index, unsigned int irq, osal_gic_handle_t handle,const char *name, void *dev);
...
extern osal_task_t *osal_kthread_create(threadfn_t thread,void *data, char *name);
extern void osal_kthread_destory(osal_task_t *task, unsigned int stop_flag);
extern void osal_kthread_bind(osal_task_t *task, unsigned int cpu);
...
extern char * osal_skip_spaces(const char *s);
extern char *osal_strim(char *s);
extern char *osal_strstrip(char *str);
...
extern char *osal_itoa(int n, char * buf);
...
extern void *osal_arch_ioaddress(void *addr);
extern void *osal_ioaddress(int addr);
#define OSAL_IO_ADDRESS(x) osal_ioaddress(x)
...
extern void osal_dma_map_area(void *addr, int size, int dir);
...
extern osal_proc_entry_t *osal_create_proc_private(const char *name, osal_proc_entry_t *parent);
extern void osal_remove_proc_private(const char *name, osal_proc_entry_t *parent);  
...
extern osal_dev_t *osal_createdev(char *name);
