...
#define HI_ISP_LSC_DEFAULT_MESH_STRENGTH    4096 //4.12
...
#define HI_ISP_LSC_DEFAULT_GAIN         (0)
#define HI_ISP_LSC_DEFAULT_GAIN_REG     (0)

#define HI_ISP_RLSC_POINTS (129)
#define HI_ISP_RLSC_DEFAULT_RADIAL_STR  4096 //4.12
#define HI_ISP_RLSC_DEFAULT_SCALE       (3)    //4.12bit
#define HI_ISP_RLSC_DEFAULT_MANUAL_WEIGHT  (256) //8 bit
#define HI_ISP_RLSC_DEFAULT_WBGAIN   (256)
#define HI_ISP_RLSC_DEFAULT_LIGHT    (0)
#define HI_ISP_RLSC_WEIGHT_Q_BITS    (8)

#define ISP_VREG_SIZE_BIN               (0x40000)
...
typedef union hiISP_MODULE_CTRL_U
{
    ...
    struct
    {
        ...
        HI_U32  bitBypassAEStaFE       : 1 ;   /* RW;[24] */
        HI_U32  bitBypassAEStaBE       : 1 ;   /* RW;[25] */
        HI_U32  bitBypassMGSta         : 1 ;   /* RW;[26] */
        HI_U32  bitRsv0                : 5 ;   /* H;[27:31] */
    };
} ISP_MODULE_CTRL_U;
/*
...
typedef struct hiISP_WDR_COMBINE_ATTR_S
{
    ...
    HI_BOOL bForceLong;          /* RW;Range:[0x0,0x1];Format:1.0; HI_TRUE: enable Force Long; HI_FALSE: disable Force Long,Only used for Hi3556AV100*/
    HI_U16  u16ForceLongLowThr;  /* RW;Range:[0x0,0xFFF];Format:12.0; Data above this threshold will Force to choose long frame only,Only used for Hi3556AV100*/
    HI_U16  u16ForceLongHigThr; /* RW;Range:[0x0,0xFFF];Format:12.0; Data below this threshold will Force to choose long frame only,Only used for Hi3556AV100*/
...
typedef struct hiISP_WDR_BNR_ATTR_S
{
    ...
    HI_BOOL        bShortFrameNR;     /* RW;Range:[0x0,0x1];Format:1.0; HI_TRUE:enable the bnr of short frame;HI_FALSE:disable the bnr of short frame,Only used for Hi3556AV100 */
    HI_U8          u8ShortFrameNRStr; /* RW;Range:[0x0,0x3F];Format:8.0;the Level 2 short frame sigma weight of G channel,Only used for Hi3556AV100                             */
    HI_U8          u8MotionBnrStr;    /* RW;Range:[0x0,0x1F];Format:5.0;the sigma weight of C channel in motion areas,Only used for Hi3556AV100                                 */
    HI_U8          u8FusionBnrStr;    /* RW;Range:[0x0,0x3F];Format:3.3,Only used for Hi3556AV100                                                                              */
} ISP_WDR_BNR_ATTR_S;
...
{
    HI_U16  u16Strength;          /* RW;Range: Hi3559AV100 = [0x0, 0xFF] | Hi3556AV100 = [0x0, 0x3FF]; Strength of dynamic range compression.
...
{
    HI_U16  u16Strength;          /* RW;Range: Hi3559AV100 = [0x0, 0xFF] | Hi3556AV100 = [0x0, 0x3FF];
    ...
    HI_U16 u16StrengthMax; /* RW; Range: Hi3559AV100 = [0x0, 0xFF] | Hi3556AV100 = [0x0, 0x3FF]; Maximum DRC strength in Auto mode */
    HI_U16 u16StrengthMin; /* RW; Range: Hi3559AV100 = [0x0, 0xFF] | Hi3556AV100 = [0x0, 0x3FF]; Minimum DRC strength in Auto mode */
...
typedef struct hiISP_DRC_ATTR_S
{
    ...
    HI_U8   u8BrightGainLmt;        /*RW;Range:[0x0,0xF];Format:4.0;Bright Gain Limit*/
    ...
    HI_U8  u8SpatialFltCoef; /* RW; Range: Hi3559AV100 = [0x0, 0xA] | Hi3556AV100 = [0x0, 0x5]; Spatial filter coefficients */
...
{
    HI_U8   u8Wgt;      /*RW;Range: [0x0, 0x80];Format:1.7;Weight of Gaussian distribution*/
...
{
    HI_U32 au32lut[HI_ISP_CLUT_LUT_LENGTH];     /* RW; Range: Hi3559AV100 = [0, 4294967295] | Hi3556AV100 = [0x0, 1073741823];*/
} ISP_CLUT_LUT_S;
...
typedef struct hiISP_DP_STATIC_CALIBRATE_S
{
    ...
    HI_U16  u16CountMax;                    /* RW; Range: [0, 8192];Format 14.0; limited Range: [0, 4096*BlkNum]Limit of max number of static defect-pixel calibraiton.*/
    HI_U16  u16CountMin;                    /* RW; Range: [0, 8192];Format 14.0; limited Range: [0, u16CountMax],Limit of min number of static defect-pixel calibraiton.*/
    ...
    HI_U16      u16Count;         /* R; Range: [0, 8192];Format 14.0; Finish number for static defect-pixel calibraiton. */
...
typedef struct hiISP_DP_STATIC_ATTR_S
{
    ...
    HI_U16  u16BrightCount;         /* RW; Range: [0, 8192];Format 14.0;limited Range: [0, 4096*BlkNum],When used as input(W), indicate the number of static bright defect pixels; As output(R),indicate the number of static bright and dark defect pixels */
    HI_U16  u16DarkCount;           /* RW; Range: [0, 8192];Format 14.0;limited Range: [0, 4096*BlkNum],When used as input(W), indicate the number of static dark defect pixels; As output(R), invalid value 0 */
...
typedef struct hiISP_SHARPEN_MANUAL_ATTR_S
{
    ...
    HI_U8  u8RGain;                /* RW; Range: Hi3559AV100 = [0, 31] | Hi3556AV100 = [0, 32]; Format:5.0;Sharpen Gain for Red Area*/
    HI_U8  u8BGain;                /* RW; Range: Hi3559AV100 = [0, 31] | Hi3556AV100 = [0, 32]; Format:5.0;Sharpen Gain for Blue Area*/
...
typedef struct hiISP_SHARPEN_AUTO_ATTR_S
{
    ...
    HI_U8  au8RGain[ISP_AUTO_ISO_STRENGTH_NUM];                 /* RW; Range: Hi3559AV100 = [0, 31] | Hi3556AV100 = [0, 32]; Format:5.0;Sharpen Gain for Red Area*/
    HI_U8  au8BGain[ISP_AUTO_ISO_STRENGTH_NUM];                 /* RW; Range: Hi3559AV100 = [0, 31] | Hi3556AV100 = [0, 32]; Format:5.0;Sharpen Gain for Blue Area*/
...
typedef struct hiISP_SHARPEN_ATTR_S
{
    ...
    //HI_U8 au8LumaWgt[ISP_SHARPEN_GAIN_NUM];   /* RW; Range: [0, 127];  Format:7.0;*/
    ISP_OP_TYPE_E enOpType;
...
{
    HI_BOOL bEnable;                            /*RW; Range:[0, 1];  Format:1.0;Enable/Disable HLC module,Only used for Hi3556AV100   */
    HI_U8   u8LumaThr;                          /*RW; Range:[0, 255];Format:8.0;Only used for Hi3556AV100                             */
    HI_U8   u8LumaTarget;                       /*RW; Range:[0, 255];Format:8.0;Only used for Hi3556AV100                             */
} ISP_HLC_ATTR_S;
...
typedef struct hiISP_CR_ATTR_S
{
    ...
    HI_U16   au16NpOffset[ISP_AUTO_ISO_STRENGTH_NUM];   /* RW; Range: [0, 16383];Format 14.0;Set Noise profile value.*/
} ISP_CR_ATTR_S;
...
{
    HI_U8   u8DetailSmoothRange;      /*RW; Range:Hi3559AV100 = [0x1, 0x8] | Hi3556AV100 = [0x1, 0x7];   Format:4.0;   Fine strength for noise suppressing*/
    HI_U8   u8NonDirDetailEhcStr;  /*RW; Range:[0x0,0x7F];  Format:3.4;   Non-direction detail enhance strength, Only used for Hi3556AV100*/
    HI_U8   u8NonDirStr;           /*RW; Range:[0x0, 0xFF]; Format:4.4;   Non-direction strength, Only used for Hi3556AV100*/
    HI_U16  u16DetailSmoothStr;    /*RW;Range:[0x0,0x100]; Format:9.0;   Strength of detail smooth, Only used for Hi3559AV100*/
    HI_U16  u16DetailSmoothThrLow;    /*RW; Range:[0x0,0x3FF]; Format:10.0;  Low threshold of detail smooth, Only used for Hi3556AV100*/
} ISP_DEMOSAIC_MANUAL_ATTR_S;

typedef struct hiISP_DEMOSAIC_AUTO_ATTR_S
{
    HI_U8   au8DetailSmoothRange[ISP_AUTO_ISO_STRENGTH_NUM];      /*RW; Range:Hi3559AV100 = [0x1, 0x8] | Hi3556AV100 = [0x1, 0x7];   Format:4.0;   Fine strength for noise suppressing*/
    HI_U8   au8NonDirDetailEhcStr[ISP_AUTO_ISO_STRENGTH_NUM];  /*RW; Range:[0x0,0x7F];  Format:3.4;   Non-direction detail enhance strength, Only used for Hi3556AV100*/
    HI_U8   au8NonDirStr[ISP_AUTO_ISO_STRENGTH_NUM];           /*RW; Range:[0x0, 0xFF]; Format:4.4;   Non-direction strength, Only used for Hi3556AV100*/
    ...
    HI_U16  au16DetailSmoothThrLow[ISP_AUTO_ISO_STRENGTH_NUM];    /*RW; Range:[0x0,0x3FF]; Format:10.0;  Low threshold of detail smooth, Only used for Hi3556AV100*/
...
{
    HI_U32               u32Strength;         /* RW;Range:[0,0xFFF];Format:4.8;Manual correction strength */
...
typedef struct hiISP_INNER_STATE_INFO_S
{
    ...
    HI_U16 u16DrcStrengthActual;          /* RW;Range: Hi3559AV100 = [0x0, 0xFF] | Hi3556AV100 = [0x0, 0x3FF];Strength of dynamic range compression.
                                                           Higher values lead to higher differential gain between shadows and highlights. */
...
typedef struct  hiISP_SPECAWB_ATTR_S
{
    ...
    HI_S32                               s32LimitFactMin;            /* RW;Range:[-2147483648, 214747647];AWB statics threshold value : minimum threshold value of ratio to pixel maximum value. */
    HI_S32                               s32LimitFactMax;            /* RW;Range:[-2147483648, 214747647];AWB statics threshold value : maximum threshold value of ratio to pixel maximum value. */
    HI_S32                               s32LimitFactLimit;          /* RW;Range:[-2147483648, 214747647];AWB statics limit value : ratio to (pixel maximum value * s32LimitFactMax) */
    HI_S32                               s32FactOffset;              /* RW;Range:[-2147483648, 214747647];The offset value of the weight when calculating the AWB statics.*/
    HI_S32                               s32BFact;                   /* RW;Range:[-2147483648, 214747647];The value of specular weight when calculating the AWB statics.*/
    HI_S32                               s32ACntMin;                 /* RW;Range:[-2147483648, 214747647];Threshold for block adoption.  If the integrated value of the AWB statics is less than m_ACntMin, the block is ignored. */
    ISP_SPECKCWB_S                       stWBNeutral;                /* RW;WB gain returned when AWB calculation can not be performed. */
    HI_U8                                bLastIsFlash;               /* RW;1: The last table is used for flash. 0: Used for unknown Bv value. */
    ...
    HI_S32                               s32APercent;                /* RW;Range:(0, 100)*/
    ...
    HI_U8                                u8ElementNum;                                               /*RW; Range:[0,7];Number of connected ISP_SPECAWB_FACTTBL_ELEMENT. */
    ...
    HI_U16                               u16Fno;                                                     /*RW; F number of the len,F1.4=14,F2.8=28,F36 =360... */
} ISP_SPECAWB_ATTR_S;
