...
def export_onnx(model, im, file, opset, train, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    try:
        ...
        # print onnx result and pytorch result
        print("========================= torch and onnx result compare ====================")
        
        test_arr = np.random.randn(1, 3, 416, 416).astype(np.float32)
        # pytorch result
        pytorch_model = model.cpu().eval()
        print("pytorch result:", pytorch_model(torch.from_numpy(test_arr)))
        # onnx result
        # pdb.set_trace()
        ort_session = ort.InferenceSession(str(f))
        onnx_outputs = ort_session.run(None, {"images":test_arr})
        print("onnx result:",onnx_outputs)
